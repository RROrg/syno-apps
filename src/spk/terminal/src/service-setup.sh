...
WITH_TMUX=${wizard_use_tmux:-true}
USER_LOGIN=${wizard_login_root:-true}
EOF
}
...
service_prestart() {
  ...
  if [ "${WITH_TMUX:-true}" = "true" ]; then
    "${SYNOPKG_PKGDEST}/bin/tmux" -u new-session -d -s __init__ >/dev/null 2>&1
    [ "${USER_LOGIN:-true}" = "false" ] && "${SYNOPKG_PKGDEST}/bin/tmux" -u set-option -g default-command 'login' >/dev/null 2>&1
  ...
  [ "${USER_LOGIN:-true}" = "false" ] && TTYDARGS+=(login) || { [ "${WITH_TMUX:-true}" = "true" ] || TTYDARGS+=(bash); }
