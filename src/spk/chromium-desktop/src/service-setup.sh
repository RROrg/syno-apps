...
service_postinst() {
  ...
  echo "VNC_PASSWORD=${wizard_vnc_password:-chromium}" >"${SYNOPKG_PKGVAR}/chromium-desktop.conf"
}
...
service_prestart() {
  ...
  # 读取安装/升级时由向导设定的 VNC 密码，作为 compose 内
  # ${wizard_vnc_password:-} 的取值传入容器（VNC_PASSWORD 环境变量）。
  VNC_PASSWORD=""
  if [ -r "${SYNOPKG_PKGVAR}/chromium-desktop.conf" ]; then
    source "${SYNOPKG_PKGVAR}/chromium-desktop.conf"
  fi

  sed -i "s/{{VNC_PASSWORD}}/${VNC_PASSWORD}/g" "${COMPOSE_FILE}"
