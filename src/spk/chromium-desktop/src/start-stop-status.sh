...
# spksrc does NOT inject these into a custom SSS_SCRIPT; define them here
...
container_status() {
  [ -x "$(command -v docker 2>/dev/null)" ] || return 1
  if docker inspect -f '{{.State.Running}}' chromium-desktop 2>/dev/null | grep -q true; then
    return 0
  fi
  return 1
}
